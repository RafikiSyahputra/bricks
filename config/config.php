...
function addToCart($data)
{
    ...
    if($banyak == 0){
        echo "<script>
                    alert('Masukkan jumlah barang yang sesuai');
                </script>";
        return false;
    }
...
function UpdateCart($data)
{
    ...
    $banyak = $data["banyak_baru"];
    $cart = query("SELECT * FROM carts WHERE id_cart = $cart_id");
    ...
    $data_product = query("SELECT * FROM products WHERE id_product = $product_id");
    ...
    if ($stock >= 1) {
        $total = $banyak * $data_product["price"];
        $queryCart = "UPDATE carts 
                      SET banyak = '$banyak', total = '$total'
...
function konfirmasi($data)
{
    ...
    // Loop through the product_id and banyak arrays
    foreach ($data["product_id"] as $key => $product_id) {
        $id_transaction = $data["id_transaction"];
        $banyak = $data["banyak"][$key];
        ...
        // If stock is available, update transaction status and product stock
        if ($sisaBarang >= 0) {
            $status = "TERKONFIRMASI";

            // Update transaction status
            $query = "UPDATE transactions SET transaction_status = '$status' WHERE id_transaction = $id_transaction";

            // Update product stock
            $queryProduct = "UPDATE products SET stock = '$sisaBarang' WHERE id_product = $product_id";
...
function upload()
{
    ...
    if ($error === 4) {
        ...
        console.log('Error: Pilih Gambar Terlebih dahulu');
        </script>";
        return false;
    }

    // Tambahkan log
    echo "<script>console.log('Temp Name: " . $tmpName . "');</script>";
    ...
    if (!in_array($ekstensiGambar, $ekstensiGambarValid)) {
        echo "<script>
                ...
                console.log('Error: Format gambar harus (png, jpg atau jpeg)');
            </script>";
        return false;
    }
    ...
    if ($size > 10000000) {
        echo "<script>
                ...
                console.log('Error: Ukuran file gambar harus < 10 Mb');
            </script>";
        return false;
    }
    ...
    // Tambahkan log
    echo "<script>console.log('Nama File Baru: " . $namaFileBaru . "');</script>";
